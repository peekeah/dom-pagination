...
container.setAttribute('class', 'container my-5 py-5 px-5');
container.innerHTML = `
  <div class="user-list"></div>
  <div id="buttons" class="d-flex justify-content-center gap-2"></div>
`
document.body.append(container);

document.querySelector(".user-list").innerHTML = `
    <h1 id="title">Users</h1>
    <p id="description" class='text-center' py-3>Pagination using DOM manipulation</p>
...
function getUsers() {
  const paginationPath = document.querySelector("#buttons");  
  let userPerPage = 10;
  let totalPages = Math.ceil(userData.length/userPerPage);
  let currentPage = 1;

  function slicedData(currentPage) {
    const newData = userData.slice((currentPage-1)*userPerPage, currentPage*userPerPage);
    return newData;
  }   

    //First Button 
    const first = document.createElement("button")
    first.setAttribute("class","first-btn");
    first.innerText = "First";
    paginationPath.append(first);
    first.onclick = () => {
      currentPage = 1;
      console.log("clicked first");
      const pageUsers = slicedData(currentPage);
      console.log(pageUsers, currentPage);
    ...
    for(let i=1; i<=totalPages; i++) {
        ...
        //logic for hiding previous and next buttons
          if(currentPage>1){ 
            previous.setAttribute("style","display: inline-block");
          }
          else{
    ...
    last.setAttribute("class","last-btn");
    last.innerText = "Last";
    ...
    last.onclick = () => {
      ...
      //default data to show on page
      const defaultData = slicedData(currentPage);
      defaultData.forEach(user => createUser(user));
}
